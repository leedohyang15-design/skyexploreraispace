...
- ✅✅✅ **[2026-08-12 대폭 갱신 — 자작 모델 파이프라인이 열렸다] probe_model_pipeline.py / probe_model_scale.py**
  ...
  · ⚠️ **`modelRadius` = 바운딩박스 반대각선의 절반 (원점까지의 거리가 아니다)**. 천리안 모델 실측으로 확정 —
    bbox 8.8×2.58×5.2m 의 반절 벡터 (4.40, 1.29, 2.60) 크기 = **5.2711**, 엔진 보고값 5.27155 와 일치.
    원점 기준 최대거리(7.69)와는 다르다 → 비대칭 모델은 배율 역산에 주의.
  · ⚠️ **`parentRadius` 는 0.0 이 나온다** (지구 EquatorialJ2000 에 붙인 뒤 읽어도) — 프레임 길이 단위의 근거로 못 쓴다.
...
- ✅✅ **바늘 텍스처 교체 확정 (2026-07-22 사용자 스샷, clock_textures.py)**: `setHoursHandTexture` / `setMinutesHandTexture` / `setSecondsHandTexture` **각 인자 1개(경로만, ⚠️ Anim 없음 — Anim 넘기면 시그니처 에러)**.
  유저폴더 PNG(투명배경) 걸면 **바늘이 그 이미지 모양으로 바뀜**(주황 다이아 바 확인). 전부 (str) 단일인자.
  ✅✅ **`setForegroundTexture`(앞 레이어=눈금/숫자) 도 확정 (2026-07-22 사용자 스샷)**: 투명배경 PNG(금테+숫자)로 걸면 문자판 눈금/숫자가 그 이미지로 바뀜.
  ⚠️ **`setBackgroundTexture`(뒤판)는 안 보임** — SystemClock001 모델 불투명 문자판이 덮음. 문자판 커스텀은 **foreground** 로.
  ⚠️ **문자판 숫자가 좌우반전**돼 보임 = FixedForeground 오버레이의 뷰-레벨 미러(Insert2D/InsertText 와 동일 현상, 세터 탓 아님).
...
- ✅✅ **[정리] '미개척 클래스' 스윕 결론 (2026-07-22, 2026-07-30 정정)**: 완본 전 클래스 중 안 해본 것들 판별 완료.
  ...
  ⚠️⚠️ **[반례 — 교훈 ②를 만능으로 쓰지 말 것 (2026-08-10 천왕성 고리 재검증)]**: 위 ⓐⓑⓒ 를 **이미 다 잡은 뒤에도** 세터가 진짜로 무력한 경우가 있다.
  천왕성 고리를 양방향(1.5↑ / 0.25↓)으로 재보니 **원반과 고리가 비례해 같이 움직여 비율이 고정** = intensity 로는 대비를 못 만든다(setScale 도 같은 이유로 체감 없음).
  즉 2026-07-30 의 '천왕성 고리 intensity 1.5 로 또렷해짐'은 **대비가 아니라 화면 전체 게인을 또렷해진 걸로 오독한 것**이었다(폐기).
  → **판정 순서를 이렇게 못박는다**: ⓐⓑⓒ 를 먼저 점검하되, **그걸 다 맞춘 뒤엔 반드시 '반대 방향'으로도 흔들어 볼 것.**
    올려서 좋아 보이는 건 게인일 수 있고, **내렸을 때 대상만 남아야 진짜 대비 레버**다. 한 방향 A/B 만으로 '레버 확정'을 선언하지 마라.
...
- 🚨🚨🚨 **[2026-08-13 돔 실측 — 3번 만에 찾은 진짜 원인] '대상이 화면에 안 보인다' 면 각도보다 **프레임**을 먼저 의심하라.**
  천리안 쇼에서 위성이 안 보인다는 지적을 받고 **두 번 연속 '가림 각도(B)' 로 진단해 둘 다 틀렸다.**
  진짜 원인: 그 장면만 **카메라를 관성 프레임(EquatorialJ2000)에 세우고 경도를 0** 으로 줬는데,
  위성은 **동기 프레임(EquatorialSynchronous)의 경도 128.2** 에 놓여 있었다.
  ⚠️⚠️ **두 프레임의 경도는 서로 아무 관계가 없다** — 동기 경도는 '지구 경도'지만 관성 경도는 춘분점 기준이고,
  둘의 차이는 **시각에 따라 달라진다.** 즉 카메라는 위성이 없는 쪽을 보고 있었고, **각도를 아무리 만져도** 안 보인다.
  ✅ **규칙**: 대상과 카메라는 **같은 프레임에서 좌표를 논해야 한다.** 동기 프레임에 놓은 위성을 보려면
  **카메라도 동기 프레임에 세우고 경도를 대상과 같게** 준다(그러면 늘 앞에 있어 가림 계산도 필요 없다).
  ⚠️ 대가: 동기 프레임에서는 지구 자전이 안 보이고 **터미네이터만 쓸린다** — '세월이 흐른다' 류 장면엔 오히려 맞다.
  ✅ **프레이밍은 감이 아니라 계산으로**: 위성 겉보기 = 2·atan(위성반지름 / 카메라–위성 거리).
  L 128.2 · B 62 · R 9.0 = **11.4°**(작다) / L 128.2 · **B 16** · R 8.6 = **31.9°**(크다).
  각도를 바꾸기 전에 **세 줄짜리 벡터 계산으로 겉보기 크기와 화면상 이격각을 먼저 뽑아 볼 것.**

- 🚨🚨🚨 **[2026-08-13 v14 — 네 판 만에 배운 것] 지상에서 물체가 안 보이면, 물체를 만지지 말고 **조준을 없애라.**
  천리안 쇼의 로켓이 네 번 연속 안 보였다. 나는 매번 **배율과 거리**를 고쳤다(5.5e4 → 3.6e3 → 50 → 150).
  사용자가 정곡을 찔렀다 — **"로켓을 미리 만들어 놓고 그곳으로 land·이동하면 되는 걸 왜 복잡하게 생각하냐."**
  · **진짜 원인은 조준이었다.** 수동 착지(R→0 + `setOrientationHPR`) 뒤 카메라 heading 이 얼마인지 모르는데
    `setOrientationH(90)`('동쪽을 봐라')을 밀어넣었다. **낙하가 남긴 자세와 트랙볼 명령이 싸워서** 어디를 보는지
    알 수 없었고, 그 상태에서 '동쪽 11 km'에 세운 물체가 화면에 들어올 이유가 없었다.
  ✅ **정답 = 방향을 강제하지 말고 읽어라.** 착지 후 `h = cam.orientationHPR.x` 를 읽고
    **방위 = 180 − h** 로 환산해 **그 방향에 물체를 놓는다**:
    `Δ위도 = (d/6378)·cos(방위)`, `Δ경도 = (d/6378)·sin(방위)/cos(위도)` (도 단위로 변환).
    조준 명령이 아예 없으니 싸울 것이 없고, heading 이 무엇이든 물체는 **반드시 시야 안**이다.
  ⚠️ **일반 규칙: 카메라 자세를 두 가지 방식(`setOrientationHPR` 과 `setOrientationH`)으로 연달아 건드리지 마라.**
    한쪽만 쓰거나, 아니면 **읽어서 거기에 맞춰라.**
  ⚠️ 곁가지지만 실제 함정: **관측자 고도 10 m 면 지평선이 11 km** — 그 거리의 물체는 밑동이 잘린다.
    지상 물체를 세울 땐 관측지 고도를 **300 m 쯤(지평선 62 km)** 으로 올려 두는 게 안전하다.

- 🚨🚨🚨 **[2026-08-13 스샷으로 확정 — '천체가 안 보인다'의 세 번째 원인은 '조명'이다]**
  천리안 쇼에서 위성이 안 보인다는 지적을 받고 **각도(B)** 를 두 번, **프레임**을 한 번 고쳤다.
  세 번째 스샷(HUD `L 128°12'E · B 16°N · R 54,852 km`)에서 **카메라는 정확히 제자리**였는데
  **지구가 초승달**이었다 — 즉 **밤면을 보고 있었고, 위성도 그늘에 들어가 검은 배경의 검은 물체**였다.
  · ⚠️ **동기(EquatorialSynchronous) 프레임에서 시간을 흘리면 반드시 밤을 지난다.** 우리는 경도에 고정인데
    **태양 직하점은 시간당 15° 서쪽으로** 가기 때문이다.
  · ✅ **계산 한 줄로 미리 안다: 직하점 경도 = 180 − 15 × UTC시각.**
    관측 경도와 **80° 안쪽이면 낮**, 100° 밖이면 밤. (128.2°E 기준: 03:30 → 0.7°(정오) ·
    08:30 → 76°(늦은 오후) · **15:00 → 173°(한밤)**.) → 동기 프레임 장면의 **시간 흐름 폭은 이 식으로 정한다.**
  · ✅ **자작 모델은 `emissionColor` 로 자체발광시켜 두면 조명과 무관하게 늘 보인다.**
    궤도 고리(emission = 색 그대로)가 어느 각도에서나 보였던 이유가 이것이다. 위성은 0.12 배로 구워
    밤면에서 사라졌다 → **0.55 배로 올려 해결.** 쇼에서 '항상 보여야 하는' 물체는 처음부터 이렇게 굽는다.
  ⚠️⚠️ **교훈: '안 보인다' 의 원인은 ① 프레임(좌표계가 다름) ② 각도(가림) ③ 조명(그늘) 셋이다.**
  나는 ②를 두 번, ①을 한 번 고치고 나서야 ③에 도달했다. **다음엔 스샷의 '지구 위상'부터 볼 것** —
  초승달이면 밤면이고, 그 자리에 있는 것은 무엇이든 안 보인다.

- 🚨🚨🚨 **[2026-08-13 확정 — '안 보인다'의 **네 번째이자 가장 흔한** 원인 = 시선축 이탈각(off-axis).
  ①프레임 ②가림 ③조명을 다 고친 뒤에도 안 보였고, 세 번의 신고가 **전부 이 계산 하나**였다.]**
  사용자: **"이거 원래 보이지도 않는 거, 내가 줌 아웃해서 보인 거야."**
  · **시선축 = 지구(트랙 대상) 중심 방향**이고, 그 축이 돔 고도 **`setTargetHeight` 값(관람 표준 30)** 에 찍힌다.
  · 대상이 적도(B=0)의 궤도에 있는데 **카메라만 B 로 올라가면**, 카메라에서 본 '중심 방향'과 '대상 방향'이
    벌어진다. 그 사잇각 θ 만큼 대상은 **돔에서 아래로** 내려간다 → 화면 고도 ≈ **Target − θ**.
    **θ > Target 이면 돔 밖**이다. 크기·배율·조명을 아무리 고쳐도 절대 안 보인다.
  · ✅ **계산(같은 경도, Rc=카메라 R, Rs=대상 R, 지구반지름 단위)**:
    `cam=(Rc·cosB, Rc·sinB)`, `sat=(Rs, 0)` → `θ = ∠(cam→sat, cam→원점)`
    정지궤도(Rs 6.611) 실측 대조: **B 62·R 9.0 → θ 44.7°(고도 −14.7 ❌)** ·
    **B 16·R 8.6 → θ 39.1°(고도 −9.1 ❌)** · **B 0·R 8.6 → θ 0°(고도 30 ✅ 정중앙)**.
  · ⚠️⚠️ **B 를 '가림 회피용'으로 올리는 습관이 바로 이 사고의 원인이었다.** 가림(orbitR·cosB > 1)은
    **대상이 카메라와 지구 사이에 있으면 애초에 일어나지 않는다.** 같은 경도에 서면 늘 앞에 있다.
    → **궤도 물체를 보여줄 때의 기본은 `B = 0`**(대상과 같은 위도면). 각도를 올릴 이유가 없다.
  · ✅ **덤: 이탈·이동을 보여줄 땐 B=0 이 오히려 연출로 맞다.** B 를 주면 이동이 '상하'로 섞여 돔 밖으로
    밀리지만, **B=0 이면 전부 '좌우'로만 벌어진다**(돔 좌우 여유는 90°). 사용자가 원한 그림 그대로다 —
    "지구를 태양처럼 가운데 두고 궤도가 옆으로 이동하는 거." (정지→폐기 이탈이 좌우 16.9°→34.6°, 고도 30 고정.)
  ⚠️ **'안 보인다' 점검 순서(확정)**: ① 프레임(대상과 카메라가 같은 좌표계인가) → ② **시선축 이탈각 θ vs Target**
    → ③ 조명(지구가 초승달인가) → ④ 가림. **크기·배율은 맨 마지막에 의심할 것.**
