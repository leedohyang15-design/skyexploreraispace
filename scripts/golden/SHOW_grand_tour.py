...
def say(text, hold=0.0):
    """자막 교체(+선택적 홀드).
       ⚠️ [2026-08-10 돔 피드백] 홀드는 **2초 + 글자당 0.1초**가 적정이다(20자 ≈ 4초).
          그 이상 붙잡으면 화면이 멈춘 것처럼 보인다 — "인터벌이 길다"는 지적을 받았다."""
    if t1 is not None:
...
# ⚠️ [2026-08-10] 예전엔 sleep(51.0) 통째로 기다렸다 — 위성이 도는 게 보여도
#    자막이 51초간 안 바뀌면 그 구간이 제일 비어 보인다("인터벌이 길다"). 9초쯤마다 갱신한다.
say("시간을 감아 보자 — 8일치를 1분 만에")
dm.setDateTime(2026, 1, 23, 12, 0, 0, tz, Anim(50.0))
sleep(6.0)
say("안쪽 이오가 가장 빠르다", 7.0)
say("유로파는 그 절반 속도", 7.0)
say("가니메데 — 태양계에서 가장 큰 위성", 7.0)
say("바깥 칼리스토는 느릿하게", 7.0)
say("멀수록 느리다", 7.0)
sleep(5.0)
