...
#  구성
#    막0  지상에서                                   (~24초)
#    막1  접근 — 황도 프레임 · L=0 · B=0             (~16초)
#    막2  세로로 선 궤도                             (~34초)
#    막3  0→90° 열기 — 세로가 과녁이 된다            (~26초)
#    막4  시간가속 — 궤도를 도는 위성                (~62초)
#    막5  정면에서 본 고리                           (~26초)
#    막6  마무리                                     (~12초)
# ══════════════════════════════════════════════════════════════════════════
...
try:
    ...
    say("궤도가 '세로'로 서 있다", 7.0)
    say("다른 행성이라면 이 각도에서 옆으로 누워 보인다", 7.5)
    say("행성이 쓰러졌으니 위성도 함께 쓰러진 채 돈다", 7.5)
    say("미란다 · 아리엘 · 움브리엘 · 티타니아 · 오베론", 5.0)
except Exception as e:
    print("막2 오류:", e)
...
try:
    ...
    sleep(6.5)
    say("정면으로 마주 봐야 비로소 보인다 — 동심원 여러 겹", 7.5)
...
try:
    ...
    sleep(5.5)
    say("누운 채로 도는 행성, 함께 누운 다섯 위성", 5.5)
