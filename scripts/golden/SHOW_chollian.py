...
#      ✅ **로켓 크기·항로 확정**: 경도 오프셋은 각도라 저고도에서 거리가 짧아진다
#         (9° 고정 → 출발 겉보기 146°). 올라가며 33°→8° 로 좁혀 ~6,000km·36~38° 로 일정하게.
#      ✅ **로켓은 R=1.0(지표)에서 출발** — 카메라용 R_LAUNCH(1.6)를 같이 쓰다가 공중에 떠 있었다.
...
#  ⚠️ **발사대(런치 패드)는 안 만들었다**: 로켓은 지표에 서지만 카메라는 3,825km 위에 있다.
#     그 거리에서 실제 크기의 발사대는 화면에서 점도 안 된다(로켓조차 4,000km로 과장해 놓았다).
#
#  구성 (대본의 시간표를 따른다)
...
# ★ 로켓 항로 — (경도 오프셋, R). ⚠️⚠️ **첫 R 은 1.0 = 지표다.**
#   [2026-08-13 돔] 로켓이 **지면에 안 붙고 떠 있었다** — 카메라용 `R_LAUNCH`(1.6 = 고도 3,825km)를
#   로켓에도 같이 써서 로켓이 카메라와 같은 높이에 떠 있었다. **발사체는 R=1.0 에서 시작해야 한다.**
#   ⚠️ 모델 원점이 **엔진면(z=0)** 이라 R=1.0 이면 밑동이 지표에 닿고 위로 선다(노즐 3m만 잠긴다).
#   올라갈수록 경도 오프셋을 좁혀 카메라와의 거리를 ~6,000km 로 유지한다(겉보기 36~38° 일정).
ROCKET_WAY = [(-33.0, 1.0), (-17.0, 3.0), (-11.0, 5.0), (-8.0, GEO_R)]
...
try:
    ...
    if ROCKET_MODEL:
        ...
        feat(rocket, "setPositionLBR",
             Vec(ROCKET_LON, 0.0, ROCKET_WAY[0][1]), Anim(0.0))   # ★ 지표에서 출발
    txt = sub_space()
