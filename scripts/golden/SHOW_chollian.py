# -*- coding: utf-8 -*-
# ─────────────────────────────────────────────────────────────
#  검증: 미확인(v2) — v1 을 2026-08-12 돔에서 돌려 **네 가지가 깨진 것을 확인**하고 고친 판이다.
#        v1 실패: ① 우주 장면 4분 내내 자막이 안 뜸 ② 지구에 색이 없음(가까이 가면 아예 사라짐)
#                 ③ 카메라가 한 번도 안 움직임 ④ 궤도선을 구분할 수 없음 + 막3 이 텅 빈 우주
#        v2 는 그 네 가지를 고쳤으나 **다시 돌려본 적은 없다.**
...
#     → 관성 프레임으로 옮기고(지구가 실제로 자전한다) 긴 단일 Anim 으로 아주 느리게 민다.
#
...
def geo_view():
    ...
    # ⚠️ [v1 버그 ③] 관성 프레임(EquatorialJ2000)으로 옮긴다.
    #    FadeTo 도킹 프레임은 카메라가 지구 자전을 따라 돌아 **지구가 멈춰 보인다.**
    #    같은 L/B/R 로 재발행하면 카메라는 안 움직이고 프레임만 바뀐다(검증된 방법).
    ip = earth.portId(Planet.PlanetPort.EquatorialJ2000)
    cam.setPositionLBR(Vec(0.0, B_TOP, R_TOP), Anim(0.0), ip)
    _dark()
...
try:
    ...
    cam.setPositionLBR(Vec(0.0, B_TOP, R_PUSH), Anim(35.0), ip)   # 아주 느린 밀어넣기
    _dark()
...
try:
    ...
    cam.setPositionLBR(Vec(0.0, B_TILT, R_PUSH), Anim(70.0), ip)
...
try:
    ...
    cam.setPositionLBR(Vec(0.0, B_NEAR, R_NEAR), Anim(0.0), ip)
    _dark()
    cam.setTargetHeight(30.0, Anim(0.0))
    _dark()
...
try:
    ...
    cam.setPositionLBR(Vec(0.0, 70.0, R_TOP), Anim(0.0), ip)   # 궤도 조망으로 복귀
    _dark()
    cam.setTargetHeight(30.0, Anim(0.0))
    _dark()
