...
# ⚠️⚠️ **로켓을 하늘로 세우는 자세 — 아직 미확정.**
#   모델은 +Z 로 서 있는데 고리 실측에서 **모델 +Z = 북극**이었다. 적도에서 바깥(하늘)을
#   향하려면 눕혀야 한다(pitch 90). 남은 건 heading 을 경도에서 어떻게 뽑느냐다.
#   ⚠️ 1차 판별(make_ariane5 의 A~E)은 **로켓 하나를 옆에서** 봐서 어안 왜곡 때문에 못 갈랐다.
#   → **`scripts/study/probe_rocket_hpr.py` 로 다시 가른다** — 같은 공식을 경도 8곳에 걸고
#     북극 위에서 본다. 맞으면 **바퀴살**, 틀리면 **바람개비**. 모양이 갈려서 눈금이 필요 없다.
#   판별 결과 번호를 아래 네 줄 중 하나로 바꾸면 끝난다:
#      1) Vec(ROCKET_LON, 90, 0)        2) Vec(-ROCKET_LON, 90, 0)
#      3) Vec(ROCKET_LON + 90, 90, 0)   4) Vec(ROCKET_LON - 90, 90, 0)
ROCKET_HPR = Vec(ROCKET_LON + 90.0, 90.0, 0.0)   # 계산상 가장 유력한 3번으로 두었다
