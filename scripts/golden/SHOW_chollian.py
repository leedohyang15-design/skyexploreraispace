...
#
#  ══ v17 — "줌 아웃해야 보였다" + "부품 이름 이거 맞냐" (2026-08-13) ══
#  ⚠️⚠️⚠️ **세 번의 '안 보인다'가 전부 같은 계산 하나였다 — 시선축 이탈각(off-axis).**
#     시선축 = 지구 중심 방향이고 그게 돔 고도 **Target(30)** 에 찍힌다. 위성은 적도(B=0)에 있는데
#     카메라만 B 로 올라가면 둘이 벌어지고, 그 각 θ 만큼 위성이 **돔에서 아래로** 내려간다(고도 ≈ 30−θ).
#       B 62·R 9.0 → θ 44.7° → 고도 **−14.7** ❌   B 16·R 8.6 → θ 39.1° → 고도 **−9.1** ❌
#       B  0·R 8.6 → θ  0.0° → 고도 **30** ✅  ← Scene 2 가 한 번도 안 깨진 이유
#     → **Scene 3 `B_TOGETHER` 16 → 0**, **Scene 4 `B_LEAVE` 8 → 0.**
#     Scene 4 는 덤으로 사용자가 말한 그림이 된다 — 이탈이 **상하가 아니라 좌우로** 벌어진다
#     (좌우 16.9° → 34.6°, 고도는 30 고정) = "지구를 태양처럼 가운데 두고 궤도가 옆으로 이동".
#  ✅ **부품 이름은 맞았다. 틀린 건 화살표 자리였다.** 다섯 이름 모두 실제 COMS-1 부품이다.
#     좌표를 감으로 넣어서 솔라세일 선이 빈 공간을, GOCI 선이 본체 아래를 짚고 있었다.
#     → 사용자 스샷으로 **화면 축을 재고(+X = 오른쪽 3.6°/m + 위 3.1°/m, +Z = 위 5.3°/m)**
#       모델 좌표에서 다섯 부품 위치를 **계산해서** 다시 넣었다(`PARTS` 주석에 유도 과정).
#
#  구성 (대본의 시간표를 따른다)
...
# ⚠️⚠️⚠️ [2026-08-13 v16 — 네 판의 데이터를 나란히 놓고 나서야 보였다]
#   판별   거리     높이      결과
#   v10   333 km  202 km   ✅ **보였다**(단 대기 산란으로 하늘색에 씻김)
#   v11   6.6 km  2.8 km   ❌ 안 보임
#   v12    11 km  8.4 km   ❌ 안 보임
#   v14    11 km  8.4 km   ❌ 안 보임 — **조준까지 고쳤는데도** 결과가 안 변했다
#   → 조준·배율·자세를 다 바꿔도 결과가 그대로였고, **변한 건 거리 하나뿐**이다.
#     그리고 **보인 건 먼 것 하나뿐**이다. = **지상 Sky View 의 근평면(near clip)이 천문 스케일이라
#     수십 km 안쪽 물체는 통째로 잘려 나간다**고 보면 네 판의 데이터가 전부 맞아떨어진다.
#   ✅ **그래서 v10 의 거리로 돌아간다(300 km).** '투명해 보였던' 건 거리 탓이 아니라 **자체발광이 약해서**였다
#     — 모델의 `emissionColor` 를 0.12 → **0.55** 로 올려(궤도 고리가 늘 또렷한 이유가 이것) 그 문제만 따로 잡는다.
#     ⚠️ **`make_chollian_model.py` 가 아니라 `make_ariane5.py` 를 다시 돌려야 반영된다.**
ROCKET_SCALE = 3.0e3          # 모델 높이 56.2m × 3000 ≈ 169 km
ROCKET_DIST_KM = 300.0        # ★ 근평면 밖. v10 에서 유일하게 보였던 거리대
ROCKET_LON_OFF = 2.7          # (heading 을 못 읽었을 때 쓰는 기본값 = 동쪽 ≈300 km)
GROUND_ALT_M = 300.0
...
# ⚠️⚠️⚠️ [2026-08-13 v17 — 세 번의 '안 보인다'가 전부 같은 계산 하나였다]
#   사용자: **"이거 원래 보이지도 않는 거, 내가 줌 아웃해서 보인 거야."** (B16·R8.6 장면)
#   내가 놓친 것은 겉보기 크기가 아니라 **시선축에서 벗어난 각(off-axis)** 이다.
#     · 시선축(카메라가 보는 축) = **지구 중심 방향**이고, 그 축이 돔 고도 **Target(=30)** 에 찍힌다.
#     · 위성은 적도(B=0)에 있는데 카메라만 B 로 올라가면, 카메라에서 본 '지구 중심'과 '위성'이 벌어진다.
#       그 사잇각 θ 만큼 위성은 **돔에서 아래로** 내려간다 → 화면 고도 ≈ **30 − θ**.
#     · θ > 30 이면 **돔 밖**이다. 크기·조명·프레임을 아무리 고쳐도 안 보인다.
#   계산(Rc=카메라 R, Rs=위성 R=6.611, 같은 경도):
#       cam=(Rc·cosB, Rc·sinB), sat=(Rs, 0) → θ = ∠(cam→sat, cam→원점)
#     B 62 · R 9.0 → θ **44.7°** → 화면고도 −14.7  ❌ (v12 "줌 땡기면 안 보인다")
#     B 16 · R 8.6 → θ **39.1°** → 화면고도  −9.1  ❌ (v13~v15 "줌 아웃해야 보였다")
#     B  0 · R 8.6 → θ   **0.0°** → 화면고도  30.0  ✅ 시선축 정중앙
#   ✅ **Scene 2(B=0)만 한 번도 안 깨졌던 이유가 이것이다.** 같은 자리로 맞춘다.
#   ⚠️ 대가는 없다 — 위성이 지구 앞을 가리므로 가림 계산도 필요 없고, 2A·2B 는 경도 차 4.8°
#     덕에 **좌우로** 15.4° 씩 벌어져 나란히 선다(좌우 이탈은 돔 반지름 90° 라 여유가 크다).
B_TOGETHER, R_TOGETHER = 0.0, 8.6
...
# ⚠️⚠️ [v17] 여기도 같은 계산으로 **B 8 → 0** (위 B_TOGETHER 주석의 off-axis 규칙).
#   B 8·R 11 에서 위성은 **상하로도** 밀린다: 정지궤도 10.9° 아래(화면고도 19) →
#   폐기궤도로 올라가면 **22.4° 아래(화면고도 7.6)** = 돔 바닥에 처박힌다. 그래서 "잘 안 보였다".
#   **B 0 이면 이탈이 전부 '좌우'로만 벌어진다**: 좌우 16.9° → 34.6°, 화면고도는 30 고정.
#   → 사용자가 말한 그림 그대로다 — **"지구를 태양처럼 가운데 두고 궤도가 옆으로 이동하는 거."**
B_LEAVE = 0.0                 # 88(탑뷰) → 62 → 24 → 8 → **0(궤도면 안, 완전 옆)**
...
# ⚠️⚠️⚠️ [2026-08-13 v17] 사용자: **"이거 부품 이름 이거 맞냐"** (스샷 3장)
#   **이름은 맞다** — 다섯 개 다 천리안 1호(COMS-1)의 실제 부품이다:
#     태양전지판(한쪽 날개) · 솔라세일(반대편 붐 끝 반사판, 그 비틀림을 상쇄하는 균형추) ·
#     Ka 대역 통신 안테나 · MI(기상영상기) · GOCI(해양관측카메라).
#   **틀린 건 이름이 아니라 화살표가 가리키는 자리였다.** 값을 감으로 넣었고, 그래서
#   솔라세일 화살표는 본체 왼쪽 위 **빈 공간**을, GOCI 선은 본체 아래 빈 곳을 짚고 있었다.
#   ✅ 이번엔 **모델 좌표에서 계산**했다. 사용자 스샷(R 52,939km = R_WATCH_B)으로 축을 재서:
#     · 돔 az 는 **화면 아래에서 시계방향** = **양수가 화면 왼쪽**(스샷 두 점으로 검산했다)
#     · 이 자세(HPR 500/20)에서 모델 **+X(태양전지판 축) → 화면 오른쪽 3.6°/m + 위 3.1°/m**
#     · 모델 **+Z(관측기 데크 축) → 화면 위 5.3°/m** (원지점 엔진 z=−1.95 가 10.4° 아래로 찍혔다)
#   모델 좌표(make_chollian_model.py) → 화면 오프셋:
#     태양전지판 x≈3.8   → 오른쪽 13.7 · 위 11.8      솔라세일   x=−3.2  → 왼쪽 11.5 · 아래 9.9
#     Ka 접시  (0.55, z2.28) → 오른쪽 2.0 · 위 13.8   MI (0.42, z2.24) → 위 13.2
#     GOCI    (−0.42, z2.15) → 왼쪽 1.5 · 위 10.1
#   ⚠️ 이 자세에선 **관측기 데크(+Z)가 위쪽·뒤쪽을 향한다** → MI·GOCI 는 본체 위 모서리에 거의
#     붙어 3° 안쪽으로 몰린다(모델이 작아서가 아니라 두 관측기가 실제로 나란히 붙어 있다).
#     그래서 **이름표만 좌우로 크게 벌려** 선이 어느 쪽에서 오는지로 구분되게 했다.
PARTS = (
    ("태양전지판",         -14.0,  12.0,  -34.0,  24.0),
    ("솔라세일 — 균형추",   11.5, -10.0,   30.0, -18.0),
    ("Ka 대역 주안테나",    -2.0,  16.0,   -6.0,  36.0),
    ("기상 관측기 (MI)",    -1.5,  13.0,   16.0,  32.0),
    ("해양 관측기 (GOCI)",   1.5,  10.5,   28.0,  19.0),
)
_tags = {}
...
try:
    ...
    #   뒤로 갈 때마다 지구에 가려 사라졌다. 지금은 **앞으로 짧게**만 흘린다(v17: 카메라 B=0 이라
    #   위성이 늘 지구 앞에 있어 가림 자체가 없다).
...
try:
    ...
    #     카메라를 12° 옆·**B 0**(v17)에 두면 지구가 화면 한가운데, 위성이 그 옆에 서고,
    #     두 고리는 옆으로 완전히 누운 선이 된다 — 이탈이 좌우로만 벌어진다.
