# -*- coding: utf-8 -*-
# ─────────────────────────────────────────────────────────────
#  검증: 부분확인 (2026-08-10) — 자세 변경 구간의 암전 처리 확인("암전 잘됐고"). 나머지 막(줌·위상·지형)은 미확인
...
try:
    ...
    say("신월 — 보이지 않는 달")
    sleep(3.0)
    # ⚠️ [2026-08-10] sleep(31.0) 통째 대기 → 자막이 안 바뀌어 비어 보였다. 9초쯤마다 갱신.
    say("한 달을 30초에 감아 보면")
    moon.setMoonAge(29.5, Anim(30.0))  # ★ 한 번의 호출로 한 사이클
    sleep(5.0)
    say("초승달 — 해가 옆에서 비춘다", 7.0)
    say("반달, 그리고 보름달", 7.0)
    say("다시 기울어 그믐으로", 7.0)
    sleep(3.0)
...
say("그래서 지구에서는 늘 같은 얼굴만 보인다", 4.2)
