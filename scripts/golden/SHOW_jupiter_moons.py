...
#       칼리스토 공전이 16.7일이므로 **17일을 80초**에 흘려 바깥 위성도 한 바퀴가 눈에 잡히게 한다.
...
#
#  구성
#    막0  지상에서 — 1610년의 그 별                (~32초)
#    막1  접근 — 목성 곁으로                       (~18초)
#    막2  네 개의 위성                             (~30초)
#    막3  시간가속 — 17일을 80초로                 (~80초)
#    막4  마무리                                   (~15초)
# ══════════════════════════════════════════════════════════════════════════
...
ACCEL_SEC = 80.0        # ⚠️ 이전 시안은 ~30초라 너무 빨랐다
...
try:
    ...
    sleep(5.0)
    say("갈릴레이가 자기가 만든 망원경을 목성에 겨눴다", 6.5)
    say("목성 옆에 작은 별 네 개가 나란히 있었다", 6.5)
    say("며칠 뒤 다시 보니 — 자리가 바뀌어 있었다", 6.5)
    say("그 별들은 목성을 돌고 있었다", 6.0)
...
# ── 막3 : 시간가속 ────────────────────────────────────────────
#   ⚠️ 이 막이 쇼의 본론이다. 너무 빠르면 그냥 '뭔가 반짝거림'으로 보인다 — 느리게.
try:
    say("17일을 80초로 — 갈릴레이가 며칠에 걸쳐 본 것을")
    dm.setDateTime(2026, 4, 6, 13, 0, 0, tz, Anim(ACCEL_SEC))
    sleep(20.0)
    say("안쪽 이오는 1.8일에 한 바퀴 — 눈에 띄게 빠르다")
    sleep(20.0)
    say("바깥 칼리스토는 16.7일 — 느릿하게 한 바퀴")
    sleep(20.0)
    say("멀수록 느리다. 케플러가 나중에 법칙으로 정리한다")
    sleep(20.0)
except Exception as e:
    print("막3 오류:", e)
...
# ── 막4 : 마무리 ──────────────────────────────────────────────
try:
    say("작은 망원경 하나가 세계관을 바꿨다", 6.5)
    say("목성과 네 위성 — 하늘에 있는 또 하나의 태양계", 6.0)
