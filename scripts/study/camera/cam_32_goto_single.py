# -*- coding: utf-8 -*-
# [카메라 32] GoTo 단독 정밀 관찰 (cam_30 과 동일 계측 → 나란히 비교용) — 화성 하나만, 끝까지, 아무것도 안 건드림
#
# 목적: StraightGoTo(cam_30) 와 **완전히 동일한 계측**으로 GoTo 를 재보고, 둘의 진짜 차이를 확정한다.
#   · cam_30(StraightGoTo) 결과: 0~10s 자세만 회전 → 10s B=90 재배치 → 11~24s 위치만 이동 → 25s R=5.00 도킹
#     (= 회전과 이동이 '분리'된 2단계)
#   · GoTo 는 옛 노트상 '비행 중 회전이 섞인다'고 돼 있으나 **정밀 측정한 적 없음** → 이걸로 확인
#
# 이 버전:
