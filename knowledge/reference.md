...
8. 🔴 **고리 쇼는 '구도'가 8할 — 단 토성과 천왕성의 정답 거리가 서로 다르다(둘을 섞지 마라).**
   ...
   · 🔴 **고리만 따로 밝히는 API 는 없다** — `setRingIntensity` 류는 존재하지 않는다.
     🔴🔴 **[2026-08-10 밝기 A/B 실측 — 옛 기록 정정] `setIntensity` 를 올려도 고리는 안 또렷해진다.**
       네 조합(1.5/1.2 × 그림자 OFF/ON)을 돔에서 비교한 결과 사용자 판정: **"고리 밝기는 다 똑같고, 행성이 밝아서 밝아 보이는 것"**.
       즉 intensity 는 **화면 전체 게인**이라 고리/원반 대비가 안 늘고, 원반만 먼저 하얗게 탄다 → **올리는 건 순손해**.
       ⚠️ 옛 기록 "1.0→1.5 A/B 하면 고리가 또렷해진다 / 1.5 가 균형"은 **이 관찰과 배치된다. 근거로 쓰지 마라.**
       → 눈이 편한 상한은 **1.2**(사용자 채택). 고리를 보이게 하는 건 밝기가 아니라 **구도(B 개방 + 근접)** 다.
