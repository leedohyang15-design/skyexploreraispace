...
4. **공전 = 위치 이동 + 재조준** — 루프 안에서 **`setOrientationSmoothXYZR(Vec4(0,0,0,0), Anim, 포트)`** 를 3스텝마다. 이게 없으면 대상이 화면 밖으로 흘러나간다.
...
7. **표면/디테일을 보여줄 땐 그림자 OFF 3세터** — `setShadowStrength(0)`+`setShadowContrast(0)`+`setPlanetShineStrength(1)`. (예외: 위상·일식·도시광처럼 그림자가 주제일 때만 ON.)
8. **줌은 '읽은 값 × 배율' + 선형 `Anim` + 겹치기**(sleep < anim). `Anim.cubic` 을 스텝마다 쓰면 뚝뚝 끊긴다. **그리고 대상마다 줌 한계가 다르다 — 토성 고리는 줌 금지(4.6 참조).**
