...
8. 🔴 **고리 쇼는 '구도'가 8할 — 단 토성과 천왕성의 정답 거리가 서로 다르다(둘을 섞지 마라).**
   · **토성 = R 을 건드리지 마라.** FadeTo 도킹 R(≈5) 그대로 + **B 만 75 로 개방.**
     A고리 바깥지름 = **2.27 토성반지름** → 고리 시직경이 R=5 에서 54°(적당), R=3.9 에서 이미 72°(잘림), R=1.7 에서 180°(삼켜짐).
     ⚠️ **"고리를 자세히 보여줘"를 줌으로 해석하지 마라** — 다가가면 오히려 잘려서 안 보인다(실측 사고 반복).
   · 🔵 **천왕성의 '누운 자세'는 프레임을 바꿔야 보인다 (2026-08-10 스샷 확정)**: FadeTo 도킹(`EquatorialSynchronous`)과
     `EquatorialJ2000` 은 **천왕성 자신의 적도면**이 기준이라 고리가 늘 '가로'로 눕는다(98° 기울기를 프레임이 흡수).
     → **`Planet.PlanetPort.Ecliptic`(황도 프레임)으로 전환해야** 기울어진 고리가 드러난다. ⚠️ 다만 B(위도)만으론 부족하고
     **L(경도)까지 맞춰야** 진짜 '세로'가 된다 — 기울어진 타원까지는 B 로, 세로 세우기는 L 스윕으로 찾을 것.
   · 🔴 **고리만 따로 밝히는 API 는 없다** — `setRingIntensity` 류는 존재하지 않는다.
     🔴🔴 **[2026-08-10 2단 실측 확정 — 옛 기록 정정] `setIntensity` 는 고리 레버가 아니다. 올려도 내려도 소용없다.**
       ① 밝히는 A/B(1.2↔1.5): 사용자 판정 **"고리 밝기는 다 똑같고, 행성이 밝아서 밝아 보이는 것"**.
       ② 어둡히는 A/B(1.2→0.25, `uranus_ring_isolate.py`): **원반이 어두워지면 고리도 같이 어두워진다** — 둘은 연동.
       → 즉 고리는 본체 intensity 에 **비례**해 움직인다 = **고리/원반 비율이 고정** = 어느 방향으로 돌려도 **대비가 안 변한다.**
         올리면 원반만 먼저 하얗게 타고, 내리면 고리가 같이 사라진다. **양방향 다 죽은 레버.**
       ⚠️ 옛 기록 "1.0→1.5 A/B 하면 고리가 또렷해진다 / 1.5 가 균형"은 **폐기.** 그건 대비가 아니라 화면 전체 게인이었다.
       → intensity 는 **'눈이 편한 노출'을 고르는 값일 뿐**이고 그 값은 **1.2**(사용자 채택 — "밝기 측면에는 괜찮다"). 고리를 보이게 하는 건 **구도(B 개방 + 근접)** 다.
       ⚠️ **`setScale`(원본×2.5·×5) 로 키우는 것도 체감 없음**(같은 프로브 C구간, 사용자 "크기는 모르겠다").
         고리와 원반이 같이 커져 **비율이 그대로**인 건 intensity 와 같은 이유. → **더 파지 마라.**
       🔴 **결론: 천왕성 고리에 남은 레버는 구도뿐이고 그건 이미 최대치(B38·R3.2)다.**
         고리를 쇼의 '주인공'으로 세우지 마라 — **'있다'만 짧게 보여주고 넘어가는 곁들이**로 쓸 것.
         (주인공감은 같은 프레임의 **위성 궤도선** — 밝고 뚜렷하며 '누운 자세'를 고리보다 잘 전달한다.)
     💡 **[일반 규칙] 뭔가를 강조하려고 `setIntensity` 를 올리지 마라.** 대상과 그 주변이 같은 값에 묶여 있으면
       둘이 같이 밝아져 **대비가 안 늘고 밝은 쪽만 먼저 하얗게 탄다.** 강조는 밝기가 아니라
       **① 구도(각도·거리) ② 배경 제거(`Stars.setIntensity(0)`) ③ 주변만 끄기** 로 만든다.
     ⚠️ **'그림자를 켜서 밤면을 죽이면 대비가 산다'는 내 제안은 탈락했다** — 같은 A/B 에서 그림자 ON 두 조합이 밀렸다.
       반쪽이 어두워지는 대신 볼 게 줄어 오히려 나빴다. **고리 장면도 그림자는 끈 채로 둔다.**
   · **천왕성 = 반대로 다가가야 보인다.** 고리가 훨씬 어둡고(반사율 3%) 작아서 멀면 그냥 점이다 →
     **`setPositionLBR(Vec(L, 0, 3.2), Anim, 황도포트)`(근접 R3.2)** + `setIntensity(1.2)` + 그림자 OFF + Stars 0.
   · 🔵🔵 **[2026-08-10 스샷 확정] 천왕성 '누운 자세'를 결정하는 건 B 가 아니라 `L` 이다.**
     황도 프레임(`Planet.PlanetPort.Ecliptic`)에서 **B=0(황도면 안)** 으로 두고 **L 만** 바꾸면:
     · **L = 0 (또는 180) → 고리·위성궤도가 완전한 '세로 선'** = 옆에서 본 모습. ★"누워서 돈다"를 한 방에 보여주는 그림.
     · **L = 90 (또는 270) → 정면 '과녁'(동심원)** = 고리면을 마주 봄. ★**고리가 가장 잘 보이는 구도**(원반 둘레에 동심원 고리가 실제로 드러남).
     · L = 135 등 중간값 → 대각선(어중간, 쓰지 말 것).
     → **연출 정석: L=0 에서 '세로'로 놀래킨 뒤 L 을 0→90 으로 스윕**하면 세로 선이 과녁으로 열린다(= 누운 자세의 증명 + 고리 공개를 한 동작으로).
     ⚠️ 옛 `B=38` 은 이 축을 몰라서 쓰던 값이다 — L 을 잡으면 **B 는 0 이 정답**.
   · ⚠️⚠️ **[R 단위 함정] 포트를 바꾸면 R 단위도 바뀐다 — 반지름이라고 가정하지 마라.**
     실측: 황도 포트에서 `R=3.2` → HUD **163,773 km** = **6.4 천왕성반지름**. 즉 **황도 포트의 R 1 단위 ≈ 천왕성 '지름'(≈51,180 km)** 이다.
     (도킹 프레임의 '트랙 대상 반지름' 단위와 **2배 차이**.) → 프레임을 바꾼 뒤 R 을 절대값으로 쓸 거면 **HUD 로 한 번 환산을 확인**할 것.
     위성 궤도(천왕성반지름 단위: 미란다 5.1 / 아리엘 7.5 / 움브리엘 10.4 / 티타니아 17.1 / **오베론 22.8**)를 이 포트에서 담으려면 **R ≈ 그 값 ÷ 2** 로 환산해 잡는다.
   · ⚠️ **FadeTo 직후 클램프 6초는 짧다 — 도킹 애니가 아직 카메라를 끌고 있어 `Anim(0.0)` 명령이 씹힌다.**
     실측: 클램프 6초 뒤 L/B 는 들어갔는데 **R 이 653,188 → 456,422 → 163,773 으로 계속 수렴**했고, 두 번째 구도는 목표 L=45 에 못 가고 **L=4.5 에서 찍혔다**.
     → **구도를 절대값으로 꽂을 거면 `cam.positionLBR.z` 를 폴링해 값이 멈춘 뒤에** 시작할 것(또는 클램프를 10초 이상).
   · 공통: `setTargetHeight(30)` + `Stars.setIntensity(0)`(배경 검정 = 대비).
   ⚠️ **풀백(R 을 키우는 것)은 줌이 아니다** — 위성계를 담으려 뒤로 빼는 건 정상이고 고리를 자르지 않는다.
   🔴🔴 **[풀백 기준점 함정 — 2026-08-10 실측] `풀백 = 읽은 R × 3.5` 의 '읽은 R' 은 반드시 도킹 R(≈5) 이어야 한다.**
   줌인을 한 뒤에 그 작아진 R 에 배율을 곱하면 풀백이 반토막 난다.
   실제 사고: 천왕성 고리를 보려고 R 3.2 → 2.29 로 당긴 뒤 `p.z * 3.5` 를 걸어 **R=8** 이 됨.
   오베론 궤도가 **22.8 천왕성반지름**이라 바깥 위성 두 개(티타니아 17.1 / 오베론 22.8)가 화면 밖으로 나갔다.
   ✅ **안전한 방법 = 풀백은 절대값으로 지정**한다(줌 이력과 무관하게 확정):
   · 천왕성 위성계 → `setPositionR(28.0, ...)` (오베론 22.8 을 담으려면 이 정도)
   · 토성 위성계   → `setPositionR(28.0, ...)` (타이탄 20.3 까지. 이아페투스 59 는 포기)
   · 목성 위성계   → `setPositionR(17.5, ...)` (도킹 5 × 3.5, 칼리스토까지 실측 확인)
...
- **천왕성 고리 ✅**: FadeTo Uranus → 그림자 OFF → **`setPositionLBR(Vec(L, 38, 3.2), Anim, -1)`**(근접+고리면 개방) → Stars 0 → **`ur.setIntensity(1.2, Anim)`**.
  ⚠️ **밝기를 더 올리지 마라** — 고리 대비는 안 늘고 원반만 탄다(2026-08-10 A/B 실측). 고리는 **구도로** 살린다. 옛 '1.5 권장'은 폐기.
