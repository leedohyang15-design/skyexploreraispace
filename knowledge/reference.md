...
- **NGC(딥스카이) ✅ — 단 '개체마다 액션이 다름' (2026-07-30 확정)**: `NGC(NGC.NGCName.NGC2237).setIntensity(1,Anim)`+`setLabelIntensity(1)` = 제자리 ON(전 개체 공통, 지상 밤하늘 그 좌표에 뜸).
  · **여행(접근) 가능한 개체 = 작고 밀집한 행성상성운**(NGC2346/NGC2392/NGC6543/NGC7009/NGC7293 등): `DataManager.database().data(Data.Type.NgcType, "NGC 2346").action(Action.Type.GoTo).trigger()` (FadeTo/StraightGoTo/ConnectTo 도 있음) → 도착 후 `cam.setTargetHeight(30)` + `setPositionR(p.z/zoom, Anim, -1)` 줌.
  · **여행 안 되는 개체 = 크고 흐린 확산성운**(NGC2237 장미): 이동 액션이 아예 없음 → **`LookAt`(조준) + `ScaleUp` 반복 트리거(확대)** 로 접근 느낌을 냄. (NGC 는 `scale` 속성/setScale 없음 → 액션으로.)
  · ⚠️ DB 이름 형식은 **"NGC 2346"**(공백 있음)이 유효. 🛑 LOS 포트 카메라 이동은 死.
  · **쇼 만들기 전에 그 개체의 액션을 확인**: `h.action(Action.Type.GoTo) is not None` 체크 → 있으면 여행, 없으면 LookAt+ScaleUp.
