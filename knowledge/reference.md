...
- 🎬 **[쇼 연출 실측 규칙 3종 (2026-07-30 사용자 피드백)]**
  ...
  · **행성 프레임 자막은 더 아래로** — 행성이 화면 중앙~아래를 크게 차지하므로 `setPosition(Vec(0, 10, 0))` + `setDistance(20)`(setSize 금지). 지상은 `Vec(0,25,0)`+`setSize(0.052)`+`distance 1.0`.
...
- ⚠️ **[위성계는 풀백이 필요]** 가스행성 도킹 R≈5 에선 위성 궤도가 화면 밖/너무 작아 안 보인다 → `setPositionR(p.z*3.5, Anim, -1)` 풀백 + 위성 `setScale(14)` 정도로 키울 것. 시간가속도 과하면 안 됨(목성 위성계는 **+2일을 36초** 정도가 적당, +7일/20초는 너무 빠름).
