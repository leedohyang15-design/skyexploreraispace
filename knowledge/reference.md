...
8. 🔴 **고리 쇼는 '구도'가 8할 — 단 토성과 천왕성의 정답 거리가 서로 다르다(둘을 섞지 마라).**
   ...
   · 🔴 **고리만 따로 밝히는 API 는 없다** — 고리 밝기는 본체 `setIntensity` 에 묶여 있어 올리면 **원반이 새하얗게 타서 눈이 아프다**(사용자 지적).
     ✅ 눈부심을 줄이는 진짜 레버 = **그림자를 되살려 밤면을 어둡게**: `setShadowStrength(1)`+`setShadowContrast(1)`+`setPlanetShineStrength(0.1~0.3)`.
     (그림자 OFF 3세터는 '표면 전체를 보여줄 때' 규칙이지, 고리 대비에는 오히려 해가 된다 — 장면 목적에 따라 갈라 쓸 것.)
   · **천왕성 = 반대로 다가가야 보인다.** 고리가 훨씬 어둡고(반사율 3%) 작아서 멀면 그냥 점이다 →
     **`setPositionLBR(Vec(L, 38, 3.2), Anim, 프레임)`(근접 R3.2 + 고리면 개방 B38)** + `setIntensity(1.5)`(고리는 본체 intensity 에 묶임, 1.8+ 는 원반이 탐).
