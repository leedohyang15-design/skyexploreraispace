...
- ⚠️⚠️ **[행성 프레임] 대상의 '화면 높이'를 정하는 건 Target 이 아니라 `B`(위도) 다 (2026-07-30 실측)**:
  같은 `setTargetHeight(30)` 인데도 **B=90(도킹기본) → 행성이 화면 맨 아래 / B=20 → 행성이 정가운데(천정=관객 목 꺾임, 부적합)**.
  → **B 를 20 까지 내리지 말 것.** 관람 정위치(화면 하단 1/3)는 그 사이 값(≈55~70 추정, `cam_16c_framing_sweep.py` 로 확정 중).
  → 즉 암석행성 접근 시 **`setPositionB` 로 B 를 '적당히만' 내리고**(90→65 정도), Target 은 30 유지.
