...
3. **자세를 바꾸는 구간은 암전 속에서** — B 이동·프레임 전환·재정렬·풀백은 `dark()` → (정렬 전부 완료) → `light()`. 슬루가 보이면 너저분하다.
   ...
   → 순서를 못박으면: `암전 → FadeTo → sleep(5~6) → 프레임 전환 → 시선 정렬 → 풀백 → (여기서 비로소) 페이드인`.
...
8. 🔴 **고리가 주제인 쇼(토성·천왕성)에서는 `setPositionR` 줌을 아예 쓰지 마라.** FadeTo 도킹 R(≈5)을 **그대로 두고 B 만 75 로 연다.** 토성 A고리 바깥지름 = **2.27 토성반지름** → 고리 전체 시직경이 R=5 에서 54°(적당), R=3.9 에서 이미 72°(잘림), R=1.7 에서 180°(화면을 통째로 삼킴).
   ⚠️⚠️ **"고리를 자세히/제대로 보여줘" 라는 요청을 줌으로 해석하지 마라 — 고리는 '구도(B 개방)'가 8할이다.** 다가가면 오히려 안 보인다. 애써 잡은 구도를 뒤 장면의 줌이 깨뜨리는 사고가 반복됐다(실측).
...
- ⚠️⚠️ **[화면이 '뚝뚝' 끊기는 것 방지 — 프레임 복귀 구간]**: 행성/딥스카이 → 지상 복귀는 `SceneGraph().reset(1)` 이 필수인데 두 가지가 겹쳐 끊김이 생긴다.
  ...
  **FadeTo 도 밝기를 1.0 으로 되돌린다.** 그래서 암전을 한 번만 건 스크립트는 **암전이 사실상 안 걸린 상태**가 되고,
  ...
  ✅ **둘 중 하나를 택하라. 어중간한 '한 번만 암전'이 최악이다**:
  · **(A) 암전을 아예 걸지 않는다** — FadeTo 자체가 페이드 전환이라 이동은 부드럽게 덮인다.
    단 **도착 후 방향정렬 회전은 보인다**(짧은 단편 쇼는 이걸 그냥 감수한다 — 검증된 g1·g6·g7 이 그렇다).
  · **(B) 확실히 감추려면 클램프 루프로 끝까지 눌러라** — 도착 자세·프레임 전환·풀백까지 감추고 싶을 때(장편 표준):
  ```python
  uni.setGlobalIntensity(0.0, Anim(0.0))
  ...
  ```
  → **암전을 쓰기로 했으면 반드시 (B) 클램프까지 간다.** 위 '암전은 프레임이 바뀔 때만' 항목과 모순처럼 보이지만,
  그 항목은 "**불필요한 암전을 덧씌우지 마라**"는 뜻이고, 이건 "**걸기로 했으면 제대로 걸어라**"는 뜻이다.
